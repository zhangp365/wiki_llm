...
| 条目 | 摘要 | 标签 |
|------|------|------|
| [Anthropic 上下文工程](concepts/anthropic-context-engineering.md) | 上下文衰减、注意力预算、上下文检索与压缩 | `agent` `anthropic` `context` |
| [Anthropic 上下文检索](concepts/anthropic-contextual-retrieval.md) | Contextual Embeddings + BM25 + 重排序 | `agent` `anthropic` `retrieval` |
| [Anthropic Agent 评估方法论](concepts/anthropic-demystifying-evals.md) | 评估指标、LLM-as-Judge、真实场景测试 | `agent` `anthropic` `evaluation` |

### Agent 安全与生产

| 条目 | 摘要 | 标签 |
|------|------|------|
| [Anthropic Claude Code 沙箱化](concepts/anthropic-claude-code-sandboxing.md) | 安全执行 Bash 和云端运行 | `agent` `anthropic` `security` |
| [Anthropic 跨产品约束 Claude](concepts/anthropic-containing-claude.md) | Agent 安全策略、沙箱、虚拟机隔离、防护机制设计 | `anthropic` `security` `agent` |
| [Anthropic Claude Code 自动模式](concepts/anthropic-claude-code-auto-mode.md) | 自动批准机制、分类器设计、权限管理、防护策略 | `anthropic` `agent` `security` |
| [Anthropic 三起生产事故复盘](concepts/anthropic-postmortem-three-issues.md) | 路由错误、输出损坏、XLA 编译器 Bug | `anthropic` `incident` `postmortem` |
| [Anthropic Claude Code 质量报告](concepts/anthropic-april-23-postmortem.md) | 三起基础设施问题：负载均衡、批处理竞态、缓存失效 | `anthropic` `incident` `postmortem` |
